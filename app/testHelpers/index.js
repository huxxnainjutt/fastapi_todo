...
const testIfKafkaVersion = (version, versionComparator) => {
  ...
  scopedTest.only = (description, callback) =>
    scopedTest(description, callback, test.only);

  return scopedTest;
};

const testIfKafkaVersionLTE = (version) =>
  testIfKafkaVersion(version, semver.lte);
const testIfKafkaVersionGTE = (version) =>
...
flakyTest.skip = (description, callback) =>
  flakyTest(description, callback, test.skip);
flakyTest.only = (description, callback) =>
  flakyTest(description, callback, test.only);
const describeIfEnv =
  (key, value) =>
